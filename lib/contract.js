...
class ContractService {
  ...
  // Sanitizes counts to handle underflow from contract (e.g. 4294967295 -> 0)
  sanitizeCount(count) {
    const num = Number(count);
    return num > 100000000 ? 0 : num;
  }
  ...
  async getProfile(publicClient, address) {
    try {
      ...
      return {
        ...
        id: Number(profile[3]),
        postCount: this.sanitizeCount(profile[4]),
        followerCount: this.sanitizeCount(profile[5]),
        followingCount: this.sanitizeCount(profile[6]),
        ...
        .map((post, index) => {
          ...
          if (Array.isArray(post)) {
            return {
              ...
              likes: this.sanitizeCount(post[6] || 0),
              commentCount: post.length > 8 ? this.sanitizeCount(post[7] || 0) : 0,
            ...
            return {
              ...
              likes: this.sanitizeCount(post.likes || 0),
              commentCount: this.sanitizeCount(post.commentCount || 0),
        ...
        .map((user, index) => {
          ...
          if (Array.isArray(user)) {
            parsedUser = {
              ...
              id: Number(user[3]),
              postCount: this.sanitizeCount(user[4]),
              followerCount: this.sanitizeCount(user[5]),
              followingCount: this.sanitizeCount(user[6]),
            };
            ...
            parsedUser = {
              ...
              id: Number(user.id),
              postCount: this.sanitizeCount(user.postCount),
              followerCount: this.sanitizeCount(user.followerCount),
              followingCount: this.sanitizeCount(user.followingCount),
            };
          } else {
